QUnit.extend( QUnit, {
	pixelEqual: function(actual, pos, rgba, message) {
		...
		actual = Array.prototype.slice.apply(imageData.data);
		var expected = [rgba[0], rgba[1], rgba[2], (rgba[3] === undefined ? 255 : rgba[3])];
		QUnit.push(QUnit.equiv(actual, expected), actual, expected, message);
	},
	surfaceEqual: function(actual, expected, message) {
	   var expectedString = typeof expected === 'string' ? expected : expected.canvas.toDataURL();
	   var actualString = typeof actual === 'string'  ? actual : actual.canvas.toDataURL();
